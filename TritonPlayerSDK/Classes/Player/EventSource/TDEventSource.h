...
typedef void (^EventSourceEventHandler)(TDEvent *event);

// ---------------------------------------------------------------------------------------------------------------------

// Connect to and receive Server-Sent Events (SSEs).
@interface TDEventSource : NSObject
...
//
// @param URL The URL of the EventSource.
+ (instancetype)eventSourceWithURL:(NSURL *)URL timeoutInterval:(NSTimeInterval)timeoutInterval maxRetries:(NSInteger) maxRetries;
...
// @param URL The URL of the EventSource.
// @param timeoutInterval The request timeout interval in seconds. See <tt>NSURLRequest</tt> for more details. Default: 5 minutes.
- (instancetype)initWithURL:(NSURL *)URL timeoutInterval:(NSTimeInterval)timeoutInterval  maxRetries:(NSInteger) maxRetries;
